	...
	private void read(Resource xmlResource) throws IOException, DocumentException {
		...
		if (!"routes".equals(root.getName())) {
			throw new DocumentException(
					"Document does not have <routes> as its root tag.");
		}

		for (@SuppressWarnings("unchecked")
		Iterator<Element> routeIter = root.elementIterator("route"); routeIter
				.hasNext();) {
			...
			for (@SuppressWarnings("unchecked")
			Iterator<Element> keyValueIter = routeElement.elementIterator(); keyValueIter
					.hasNext();) {
				Element keyValueElement = keyValueIter.next();
				metadata.put(keyValueElement.getName(),
						keyValueElement.getText());
			}
