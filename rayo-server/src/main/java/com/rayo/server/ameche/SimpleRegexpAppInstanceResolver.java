...
/**
 * <p>
 * Resolves Ameche routing rules defined on a external file, typically
 * WEB-INF/ameche-routing.properties. Routing rules will have into consideration
 * both to/from fields. All instances matching the given offer will be
 * dispatched.
 * </p>
 * 
 * @author martin
...
public class SimpleRegexpAppInstanceResolver extends AppInstanceResolverS
	...
	public SimpleRegexpAppInstanceResolver(final Resource properties, int delay)
			throws IOException {
	...
	void read(Resource properties) throws IOException {
		...
		try {
			logger.debug("Reading Ameche routes from disk [%s]",
					properties.getFilename());
		} catch (IllegalStateException ise) {
			// Ignore. On testing a byte array does not have a filename property
			...
			if (properties.isReadable()) {
				...
				while (scanner.hasNextLine()) {
					...
					if (line.trim().length() > 0
						...
						int appInstanceId = 0;
						try {
							appInstanceId = Integer.parseInt(uri.substring(0,
									colon));
				...
				logger.warn(
						"Could not find AppInstance resolver configuration file [%s]",
						properties);
			}
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
